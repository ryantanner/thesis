...
public class WordToSentenceProcessor<IN> implements ListProcessor<IN, List<IN>> {
  ...
  public void setSentenceBoundaryToDiscard(Set<String> set) {
  	sentenceBoundaryToDiscard = new HashSet<Pattern>();
  	for(String s: set){
            sentenceBoundaryToDiscard.add(Pattern.compile(s));
  	}
  }
  ...
  public void addHtmlSentenceBoundaryToDiscard(Set<String> set) {
    ...
    for (String s: set) {
      sentenceBoundaryToDiscard.addAll(toHtmlPattern(s));
    }
  }

  private static Set<Pattern> toHtmlPattern(String tag) {
    Set<Pattern> patterns = new HashSet<Pattern>();
    patterns.add(Pattern.compile("<\\s*/?\\s*" + tag + "\\s*/?\\s*>", Pattern.CASE_INSENSITIVE));
    patterns.add(Pattern.compile("<\\s*" + tag + "\\s+[^>]>", Pattern.CASE_INSENSITIVE));
    return patterns;
  }
