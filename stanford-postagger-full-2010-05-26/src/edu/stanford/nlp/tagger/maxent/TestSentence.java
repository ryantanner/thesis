...
public class TestSentence implements SequenceModel {
  ...
  public TestSentence(MaxentTagger maxentTagger, String[] s, String[] correctTags, PrintFile pf,
                      Dictionary wrongWords, boolean verboseResults) {
    this(maxentTagger);
    if (DBG) {
  ...
  public ArrayList<TaggedWord> tagSentence(List<? extends HasWord> s) {
    ...
    if (VERBOSE) {
      System.err.println("Sentence is " +
                         Sentence.listToString(sentence, false, maxentTagger.config.getTagSeparator()));
    }
    init();
  ...
  private ArrayList<TaggedWord> testTagInference() {
    ...
    return new ArrayList<TaggedWord>(taggedWords);
  }
