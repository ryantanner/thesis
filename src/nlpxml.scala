...
def parsetree(node: ParseTreeNode, elems: List[String]): ParseTreeNode = {    
    if (elems isEmpty) { return node }
        ...
        if (node.parent != null)
            println("1\t" + elems.head + "-" + node.word + "/" + node.pos)
        node.pos = ((elems head) tail)
        if ('(' == (((elems tail) head) head)) {
            val child = new ParseTreeNode(node)           
            node :+ child
            return parsetree(child,(elems tail))
            }
        else	{
            node.word = ((elems tail) head)
            return parsetree(node.parent,(elems tail) tail)
        }

    }
    else if (elems.head == ")") {     
        println("2\t" + elems.head + "-" + node.word + "/" + node.pos)  
        return parsetree(node.parent,elems tail)     
    }
    else    {
/*
        println("3\t" + elems.head + "-" + node.word + "/" + node.pos)
        node.word = elems.head
...
// use this one!
// elems should be a list of the parse tree in format: List("(ROOT","WORD)))") 
