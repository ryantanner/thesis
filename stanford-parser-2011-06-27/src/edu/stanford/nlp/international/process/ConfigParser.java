 ...
 *
 */
public class ConfigParser implements Iterable<StringMap> {
  ...
  public static final String paramType = "TYPE";            //Specify the ArabicDataset type to use
  ...
  //Other members
  private final List<StringMap> datasetList;
  private final Map<String,Pair<Pattern,Pattern>> patternsMap;
  private final String configFile;

  public ConfigParser(String filename) {
    configFile = filename;
    datasetList = new ArrayList<StringMap>();
  ...
  public Iterator<StringMap> iterator() {
    Iterator<StringMap> itr = Collections.unmodifiableList(datasetList).iterator();
    return itr;
  }
  ...
  public void parse() {
    ...
    try {
      ...
      StringMap paramsForDataset = null;
      ...
      while(reader.ready()) {
        ...
        if(m.matches() && paramsForDataset != null) {
          ...
          paramsForDataset = new StringMap();
          ...
          if(paramToken.lookingAt()) {
            ...
            if(paramTemplate.second != null) {
              ...
              if(paramToken.matches()) {
                paramsForDataset.put(actualParam, paramValue);
              } else {
  ...
  public String toString() {
    ...
    int dataSetNum = 1;
    for(StringMap sm : datasetList) {
      if(sm.contains(paramName))
        sb.append(String.format(" %d: %s\n",dataSetNum++,sm.get(paramName)));
      else
