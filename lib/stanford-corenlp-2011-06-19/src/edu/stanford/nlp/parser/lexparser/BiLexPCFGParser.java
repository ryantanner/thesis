...
public class BiLexPCFGParser implements KBestViterbiParser {
  ...
  protected Tree extractParse(Edge edge) {
    ...
    if (edge.backEdge == null && edge.backHook == null) {
      ...
      List<Tree> childList = Collections.singletonList(tf.newLeaf(head));
  ...
  protected List<Item> makeInitialItems(List wordList) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      if(wordObject instanceof HasContext) {
        wordContextStr = ((HasContext) wordObject).current();
...
//    scoreDependencies();
  ...
  public BiLexPCFGParser(Scorer scorer, ExhaustivePCFGParser fscorer, ExhaustiveDependencyParser dparser, BinaryGrammar bg, UnaryGrammar ug, DependencyGrammar dg, Lexicon lex, Options op) {
