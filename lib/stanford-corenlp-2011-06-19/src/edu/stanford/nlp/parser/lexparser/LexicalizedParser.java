...
public class LexicalizedParser implements ViterbiParserWithOptions, Function<Object,Tree> {
   ...
   * the entire tagger.
   */
  protected static Function<List<? extends HasWord>,ArrayList<TaggedWord>> tagger;
  ...
  public Tree getBestParse(boolean stripSubcat) {
    if (bparser != null && parseSucceeded) {
      ...
      return (stripSubcat) ? subcategoryStripper.transformTree(tree) : tree;
  ...
  public final ParserData getParserDataFromTreebank(Treebank trainTreebank,
    ...
    if(Train.ruleSmoothing) {
      System.err.print("Smoothing PCFG...");
      Function<Pair<UnaryGrammar,BinaryGrammar>,Pair<UnaryGrammar,BinaryGrammar>> smoother = new LinearGrammarSmoother();
   ...
   */
  static ArrayList<? extends HasWord> getInputSentence(Tree t) {
    if (Test.forceTags) {
      if (Test.preTag) {
  ...
  public double testOnTreebank(Treebank testTreebank) {
    ...
    if (Test.writeOutputFiles) {
      String fname = Test.outputFilesPrefix + "." + Test.outputFilesExtension;
      try {
    ...
    if(Test.outputkBestEquivocation != null) {
      try {
        statsPwo = op.tlpParams.pw(new FileOutputStream(Test.outputkBestEquivocation));
      } catch(IOException ioe) {
        ioe.printStackTrace();
      }
    }
    ...
    if (Boolean.parseBoolean(Test.evals.getProperty("pcfgLA"))) {
    ...
    if (Boolean.parseBoolean(Test.evals.getProperty("depTA"))) {
      depTA = new TaggingEval("dep Tag", runningAverages, pd.lex);
    }
    ...
    if (Boolean.parseBoolean(Test.evals.getProperty("factLA"))) {
    ...
    for (Tree goldTree : testTreebank) {
      ...
      try {
        if ( ! parse(sentence)) {
          ...
          if (pparser != null && pparser.hasParse() && fallbackToPCFG) {
            ...
            tree = getBestPCFGParse(false);
          } else {
            pwErr.println();
          }
        } else {
          tree = getBestParse(false);
      ...
      if (tree != null) {
        ...
        else if (Test.printPCFGkBest > 0 && Test.outputkBestEquivocation == null) {
        ...
        //1-best output
          ...
          treePrint.printTree(tree, "0", pwo);
        }

        //Print the derivational entropy
        if(Test.outputkBestEquivocation != null && Test.printPCFGkBest > 0) {
          List<ScoredObject<Tree>> trees = getKBestPCFGParses(Test.printPCFGkBest);
          ...
          for(int i = 0; i < logScores.length; i++) {
            double logPr = logScores[i] - denom;
      ...
      if (tree != null) {
        ...
        if (transGoldTree == null) {
          ...
          pwOut.println("WARNING: Evaluation could not be performed due to guess/gold yield mismatch.");
          numSkippedEvals++;
          continue;
        }
        ...
        if (treePCFG != null) {
          ...
          if (pcfgDA != null) {
            ...
            Tree cwtPCFGTreeB = pparser.getBestParse();
            if(!(cwtPCFGTreeB.label() instanceof CoreLabel)) {
              cwtPCFGTreeB = cwtPCFGTreeB.deepCopy(new LabeledScoredTreeFactory(),CoreLabel.factory());
            }
   ...
   *  index argIndex on.
   */
  private void parseFiles(String[] args, int argIndex, boolean tokenized, TokenizerFactory<? extends HasWord> tokenizerFactory, String elementDelimiter, String sentenceDelimiter, Function<List<HasWord>, List<HasWord>> escaper, String tagDelimiter) {
    ...
    for (int i = argIndex; i < args.length; i++) {
      ...
      final DocumentPreprocessor documentPreprocessor = filename.equals("-") ?
        new DocumentPreprocessor(new BufferedReader(new InputStreamReader(System.in)),docType) :
        new DocumentPreprocessor(filename,docType);
      ...
      if (Test.writeOutputFiles) {
        ...
        if (Test.outputFilesDirectory != null && !Test.outputFilesDirectory.equals("")) {
          String fseparator = System.getProperty("file.separator");
          if (fseparator == null || "".equals(fseparator)) {
            fseparator = "/";
          }
          File fnameFile = new File(fname);
          fname = Test.outputFilesDirectory + fseparator + fnameFile.getName();
        }
      ...
      if (Test.writeOutputFiles) pwo.close();
  ...
  public static void main(String[] args) {
    ...
    if (testTreebank != null) {
      ...
      lp.parseFiles(args, argIndex, tokenized, tokenizerFactory, elementDelimiter, sentenceDelimiter, escaper, tagDelimiter);
    }
  } // end main
