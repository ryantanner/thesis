...
public class CharniakScoredParsesReaderWriter {
  ...
  private final static LabeledScoredTreeFactory treeFactory = new LabeledScoredTreeFactory();
  ...
  private static Tree readTree(String str) {
    try {
      PennTreeReader ptr = new PennTreeReader(new StringReader(str), treeFactory);
      return ptr.readTree();
    } catch (IOException ex) {
      throw new RuntimeException(ex);
    }
  ...
  {
    try {
      PrintWriter pw = FileUtils.getPrintWriter(filename);
      int i = 0;
  ...
  private static class ScoredParsesIterator extends AbstractIterator<List<ScoredObject<Tree>>>
  {
    ...
    private ScoredParsesIterator(String filename) throws IOException {
      this(filename, FileUtils.getBufferedFileReader(filename));
    }
          ...
          if (line.length() > 0) {
            if (parsesExpected == 0) {
              ...
              if (curParse == null) {
                ...
                // Reading a parse
                curParse.setObject(readTree(line));
                curParse = null;
                parsesExpected--;
                if (parsesExpected == 0) {
                  return curParses;
