  ...
  public void annotate(Annotation annotation) {
    ...
    if (annotation.containsKey(WordsPLAnnotation.class)) {
      ...
      for (List<? extends CoreLabel> words : sentences) {
        List<CoreLabel> output = trueCaser.classifySentence(words);
        // move the sequence classifier answers from the Answer to NER field
        Iterator<? extends CoreLabel> origFLIter = words.iterator();
        for (CoreLabel fl : output) {
          //System.err.println("ner()" + fl.ner());
          //System.err.println("answer()" + fl.answer());
          CoreLabel next = origFLIter.next();
          next.set(CoreAnnotations.TrueCaseAnnotation.class, fl.get(AnswerAnnotation.class));
          setTrueCaseText(next);
        }
      }
      if (VERBOSE) {
