...
public class DeterministicCorefAnnotator implements Annotator {
  ...
  public void annotate(Annotation annotation){
    ...
    if(OLD_FORMAT) {
      ...
      // this graph is later used to find the connected components for CorefClusterAnnotation and CorefClusterIdAnnotation
      UndirectedGraph<IntTuple, DefaultEdge> jgraph = new SimpleGraph<IntTuple, DefaultEdge>(
          DefaultEdge.class);
      for(Pair<IntTuple, IntTuple> link: links){
        //
        ...
        if (!src.equals(dst)) {
          jgraph.addVertex(src);
          jgraph.addVertex(dst);
          jgraph.addEdge(src, dst);
        }
      }
      annotation.set(CorefGraphAnnotation.class, graph);
      
      ConnectivityInspector<IntTuple, DefaultEdge> conn = new ConnectivityInspector<IntTuple, DefaultEdge>(
          jgraph);
      List<Set<IntTuple>> corefConnectedComponents = conn.connectedSets();

      for (Set<IntTuple> corefCluster : corefConnectedComponents) {
        Set<CoreLabel> coreferentTokens = new HashSet<CoreLabel>();
        Set<CoreLabel> cyclicCoreferentTokens = new HashSet<CoreLabel>();
        for (IntTuple tuple : corefCluster) {
          CoreMap sentence = annotation.get(SentencesAnnotation.class).get(tuple.get(0) - 1);
          CoreLabel token = sentence.get(TokensAnnotation.class).get(tuple.get(1) - 1);
