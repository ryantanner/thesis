...
public final class TreebankPreprocessor {
  ...
  private static Dataset getDatasetClass(StringMap dsParams) {
    Dataset ds = null;
    String dsType = dsParams.remove(ConfigParser.paramType);
    try {
      if(dsType == null)
  ...
  public static void main(String[] args) {
    ...
    for(StringMap dsParams : cp) {
      String nameOfDataset = (dsParams.contains(ConfigParser.paramName)) ? dsParams.get(ConfigParser.paramName) : "UN-NAMED";
      ...
      boolean shouldDistribute = (dsParams.contains(ConfigParser.paramDistrib)) ? 
          Boolean.parseBoolean(dsParams.get(ConfigParser.paramDistrib)) : false;
