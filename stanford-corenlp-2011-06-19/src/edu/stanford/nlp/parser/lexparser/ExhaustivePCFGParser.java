...
public class ExhaustivePCFGParser implements Scorer, KBestViterbiParser {
  ...
  private CoreLabel getCoreLabel(int labelIndex) {
    ...
    String wordStr = (String) wordNumberer.object(words[labelIndex]);
  ...
  protected boolean[] isTag = null; // this records whether grammar states (stateNumberer) correspond to POS tags
...
//    }
    ...
    //WSGDEBUG
    //I don't understand the "half-filters" bit below...the arrays are used by the bilex
    //parser during edge combination
    if (Test.verbose) {
      Timing.tick("done.");
      System.err.print("Starting half-filters...");
    }
      ...
      for (int start = 0; start < ((diff == length) ? 1: length - diff); start++) {
          ...
          for (BinaryRule r : leftRules) {
            ...
            if ( ! Test.lengthNormalization) {
              ...
              for (int split = min; split <= max; split++) {
                ...
                float tot = pS + lS + rS;
                if (spillGuts) { System.err.println("Rule " + r + " over [" + start + "," + end + ") has log score " + tot + " from L[" + stateNumberer.object(leftState) + "=" + leftState + "] = "+ lS  + " R[" + stateNumberer.object(r.rightChild) + "=" + r.rightChild + "] =  " + rS); }
                if (tot > bestIScore) {
                  bestIScore = tot;
                }
              } // for split point
            ...
            if (foundBetter) { // this way of making "parentState" is better than previous
              ...
              if (spillGuts) System.err.println("Could build " + stateNumberer.object(parentState) + " from " + start + " to " + end + " score " + bestIScore);
        ...
        for (int rightState = 0; rightState < numStates; rightState++) {
          ...
          for (BinaryRule r : rightRules) {
            ...
            if (foundBetter) { // this way of making "parentState" is better than previous
              iScore[start][end][parentState] = bestIScore;
              if (spillGuts) System.err.println("Could build " + stateNumberer.object(parentState) + " from " + start + " to " + end + " with score " + bestIScore);
              if (oldIScore == Float.NEGATIVE_INFINITY) {
                if (start > narrowLExtent[end][parentState]) {
                  narrowLExtent[end][parentState] = start;
                  wideLExtent[end][parentState] = start;
        ...
        for (int state = 0; state < numStates; state++) {
          ...
          for (UnaryRule ur : unaries) {
            ...
            if (foundBetter) {
              if (spillGuts) System.err.println("Could build " + stateNumberer.object(parentState) + " from " + start + " to " + end + " with score " + tot);
    ...
    for (int start = 0; start + 1 <= length; start++) {
      if (Test.maxSpanForTags > 1) { // only relevant for parsing single words as multiple input tokens.
        ...
        if(sentence.get(start) instanceof HasContext) {
          wordContextStr = ((HasContext) sentence.get(start)).current();
        ...
        if ( ! floodTags || word == boundary) {
          ...
          for (Iterator<IntTaggedWord> taggingI = lex.ruleIteratorByWord(word, start, wordContextStr); taggingI.hasNext(); ) {
            ...
            tags[start][tag] = true;
            if (dumpTagging) {
              EncodingPrintWriter.err.println("Word pos " + start + " tagging " + tagging + " score " + iScore[start][end][state] + " [state " + Numberer.object("states", state) + " = " + state + "]", "UTF-8");
            }
