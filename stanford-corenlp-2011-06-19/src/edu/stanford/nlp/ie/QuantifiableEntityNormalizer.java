...
public class QuantifiableEntityNormalizer {
  ...
  private static final EditDistance ed = new EditDistance();
  ...
  static {
    ...
    wordsToValues.setCount("eightteen", 18.0);
    ...
    ordinalsToValues.setCount("eightteenth", 18.0);
  ...
  private static String getOneSubstitutionMatch(String word, Set<String> set) {
    for(String cur : set) {
      if(isOneSubstitutionMatch(word, cur))
        return cur;
    }
    return null;
  }
  ...
  static <E extends CoreMap> void concatenateNumericString(List<E> words, List<E> toRemove) {
    ...
    foundEntity.set(CurrentAnnotation.class, collapsed);
  }
  ...
  public static String normalizedNumberString(String s, String nextWord) {
    if (DEBUG2) { err.println("normalizedNumberString: normalizing "+s); }
    return normalizedNumberStringQuiet(s, 1.0, nextWord);
  }
  ...
  public static String normalizedNumberStringQuiet(String s,
      double multiplier, String nextWord) {
    // clean up string
    ...
    if(!foundMultiplier) {
      for (String moneyTag : moneyMultipliers.keySet()) {
        if(isOneSubstitutionMatch(origSSplit[origSSplit.length - 1], moneyTag)) {
          s = s.replaceAll(moneyTag, "");
          multiplier *= moneyMultipliers.get(moneyTag);
        }
      }
  ...
  public static String normalizedOrdinalStringQuiet(String s) {
    ...
    if (Character.isDigit(s.charAt(0))) {
      ...
      return normalizedNumberStringQuiet(s.substring(0, s.length() - 2), 1.0, "");
  ...
  private static <E extends CoreMap> List<E> processEntity(List<E> l,
    ...
    if (DEBUG) {
      System.err.println("Quanitifiable.processEntity: " + l);
    }
    String s;
  ...
  private static <E extends CoreMap> String timeEntityToString(List<E> l) {
    ...
    int size = l.size();
    for (E w : l) {
      assert(w.get(AnswerAnnotation.class).equals(entityType));
  ...
  public static <E extends CoreLabel> List<E> applySpecializedNER(List<E> l) {
    ...
    AbstractSequenceClassifier nsc = new NumberSequenceClassifier();
