...
public class CustomAnnotationSerializer implements AnnotationSerializer {
  ...
  public Annotation load(InputStream is) throws IOException, ClassNotFoundException, ClassCastException {
    ...
    while((line = reader.readLine()) != null){
      ...
      while((line = reader.readLine()) != null){
        ...
        CoreLabel token = loadToken(line);
        tokens.add(token);
      }
  ...
  private static void saveDependencyGraph(SemanticGraph graph, PrintWriter pw) {
    ...
    // first line: sentence index for all nodes; we recover the words from the original tokens
    // the first two tokens in this line indicate: docid, sentence index
    if(graph.vertexList() != null && graph.vertexList().size() > 0){
      IndexedWord first = graph.vertexList().get(0);
      String docId = first.get(DocIDAnnotation.class);
      if(docId != null && docId.length() > 0) pw.print(docId);
      else pw.print("-");
      pw.print("\t");
      pw.print(first.get(SentenceIndexAnnotation.class));
    }
