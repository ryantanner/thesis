...
public class FrenchTreeReader implements TreeReader {
  ...
  private String getMorph(Element node) {
    String mph = node.getAttribute(ATTR_MORPH);
    if(mph == null) mph = "";
    
    //wsg2011: It seems that this annotation is a superset of mph
    String ee = node.getAttribute(ATTR_EE);
 
    return (ee != null) ? ee : mph;
//    return String.format("%s--%s",mph.trim(),ee.trim());
  }
      ...
      if(leafToks.size() > 1) {
        for (int i = 0; i < leafToks.size(); ++i) {
          ...
          if(leafNode.label() instanceof HasContext)
            ((HasContext) leafNode.label()).setCurrent(morph);
    ...
    try {
      for(File file : fileList) {
        ...
        for(numTrees = 0; (t = tr.readTree()) != null; numTrees++) {
          System.out.printf("%s-%s\t%s%n",canonicalFileName,((HasIndex) t.label()).docID(),t.toString());
          List<Label> leaves = t.yield();
          for(Label label : leaves) {
            if(label instanceof CoreLabel)
              morphAnalyses.add(((CoreLabel) label).current());
          }
        }
