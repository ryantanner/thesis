...
public class TaggerConfig extends Properties /* Inherits implementation of serializable! */ {
  ...
  DataInputStream getTaggerDataInputStream(String modelFileOrUrl) throws IOException {
    InputStream is; // initialized below
    if (modelFileOrUrl.matches("https?://.*")) {
      ...
      // ms 10-04-2010: check if we find this path somewhere in the CLASSPATH. This takes precedence over the file system.
      is = getClass().getClassLoader().getResourceAsStream(modelFileOrUrl);
      // only if not found try the file system
      if (is == null) {
